# Surat Lamaran — Data Entry / Administration / Virtual Assistant (Remote)

---

## Versi Bahasa Indonesia

**Kepada Yth. HRD / Hiring Manager,**

Dengan hormat,

Saya, **Richard Patung Landu Meha**, mengajukan lamaran untuk posisi **Data Entry / Administration Staff** di perusahaan Bapak/Ibu.

Saya memiliki **7+ tahun pengalaman** dalam entry dan pengelolaan data personel, timesheet, scheduling, dan pelaporan operasional di divisi keamanan berskala besar. Beberapa pencapaian saya:

- **Entry & maintenance database** 80+ personel (NIK, jabatan, NPWP, BPJS, lokasi tugas) — update berkala
- **Timesheet entry** 150+ personel per bulan — konsisten 36+ bulan berturut-turut tanpa jeda
- **Scheduling entry** jadwal shift dengan sistem rotasi 3 regu — diekspor ke CSV terstruktur
- **Google Forms & Sheets** — mendesain dan mengelola form pengumpulan data (alamat, NPWP, seragam) dengan dropdown nama otomatis via Apps Script
- **Reporting** — 45+ laporan mingguan, 20+ laporan bulanan, weekly report periodik

**Tools yang dikuasai:**
- Microsoft Excel (Vlookup, Pivot Table, Data Validation, Conditional Formatting)
- Google Workspace (Sheets, Forms, Docs, Drive, Apps Script)
- Google Forms Automation (Apps Script untuk auto-populate dropdown)
- HTML, CSS, JavaScript (dasar-dasar pengembangan web)

Saya juga sedang menempuh **S1 Teknik Informatika** dan **S1 Manajemen**, serta tersertifikasi **BNSP Digital Marketer**. Saya pribadi yang disiplin, teliti, dan mampu bekerja dengan target deadline.

Saya sangat antusias untuk dapat berkontribusi di perusahaan Bapak/Ibu. Terlampir CV dan portfolio online untuk referensi lebih lanjut.

Wassalamu'alaikum Wr. Wb.

**Richard Patung Landu Meha**
+62 858-8810-2696 | richardpl.meha@gmail.com
Portfolio: https://rihard-cyber.github.io/richard-portfoliofulleng/
LinkedIn: linkedin.com/in/richard-patung-landu-meha-9099a3259

---

## English Version

**Dear HRD / Hiring Manager,**

I am writing to apply for the **Data Entry / Administration Specialist** position at your company.

With **7+ years of experience** in personnel database management, timesheet processing, scheduling, and operational reporting, I bring a strong track record of accurate and consistent data entry. Key achievements:

- **Database entry & maintenance** for 80+ personnel (NIK, positions, NPWP, BPJS, locations) — regular updates
- **Timesheet entry** for 150+ personnel monthly — consistent for 36+ consecutive months
- **Shift scheduling entry** with 3-team rotation system — exported to structured CSV
- **Google Forms & Sheets** — designed and managed data collection forms with auto-populated name dropdowns via Apps Script
- **Reporting** — 45+ weekly reports, 20+ monthly reports, periodic evaluations

**Tools Proficiency:**
- Microsoft Excel (Vlookup, Pivot Tables, Data Validation, Conditional Formatting)
- Google Workspace (Sheets, Forms, Docs, Drive, Apps Script)
- Google Forms Automation (Apps Script for auto-populate dropdowns)
- HTML, CSS, JavaScript (web development basics)

I am currently pursuing dual Bachelor's degrees in **Informatics Engineering** and **Management**, and hold **BNSP Digital Marketing certification**. I am disciplined, detail-oriented, and deadline-driven.

I would welcome the opportunity to contribute to your team. Please find my CV and online portfolio attached for your reference.

Sincerely,

**Richard Patung Landu Meha**
+62 858-8810-2696 | richardpl.meha@gmail.com
Portfolio: https://rihard-cyber.github.io/richard-portfoliofulleng/
LinkedIn: linkedin.com/in/richard-patung-landu-meha-9099a3259
